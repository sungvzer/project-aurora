    ...
    describe("Signup", () => {
        ...
        test("Signup is successful", async () => {
            ...
            for (let key in signupTemplate.data.attributes) {
                if (
                    !signupTemplate.data.attributes.hasOwnProperty(key) ||
                    key === "password"
                ) {
                    break;
                }
            ...
            describe("Validity checks", () => {
                test("Invalid Authorization/No valid JWT", async () => {
                    let response = await request(app)
                        ...
                        .set(
                            "Authorization",
                            `Bearer djwoeihwgrf823hrngf2ync873rcy782tb78`
                        )
                        .send(req);
    ...
    describe("Deletion", () => {
        ...
        describe("Authorization", () => {
            ...
            test("Fails if requested user id is not the same as logged user id", async () => {
                ...
                await User.delete(
                    (
                        await User.getUserIdByEmail(newEmail)
                    ).value
                );
            });
        });
