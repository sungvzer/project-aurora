...
export const getUserTransactions = async (req: Request, res: Response) => {
    ...
    if (req.query.startDate)
        queryOptions.startDate = new Date(
            Date.parse(req.query.startDate.toString())
        );

    if (req.query.endDate)
        queryOptions.endDate = new Date(
            Date.parse(req.query.endDate.toString())
        );
    ...
    if (!req.params.trId) {
        ...
        transactionsOrError = await User.getTransactionsByUserId(
            payloadID,
            queryOptions
        );
        ...
        response.data = transactionsOrError.value.map<ResourceObject>(
            (value) => {
        ...
        let transactionOrError = await User.getTransactionById(
            parseInt(req.params.trId)
        );
