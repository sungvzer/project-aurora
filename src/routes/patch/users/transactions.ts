...
export const patchUserTransaction = async (req: Request, res: Response): Promise<Response> => {
    ...
    await param("trId", err.invalidTransactionId).isInt({ allow_leading_zeroes: false, gt: 0 }).run(req);
    ...
    if (currency) {
        ...
        sql += ' UserTransactionCurrencyID = (SELECT CurrencyID FROM Currency WHERE CurrencyCode = ?) ';
    ...
    return res.status(201).header("Location", `/users/${userId}/transactions/${transactionID}`).json(response.close());
};
