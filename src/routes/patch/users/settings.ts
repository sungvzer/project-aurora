...
export const patchUserSettings = async (req: Request, res: Response): Promise<Response> => {
    ...
    await check("data", err.invalidDarkModeValue).custom((input) => {
        ...
        return validator.isInt(value.toString(), { allow_leading_zeroes: false, min: 0, max: 1 });
    }).run(req);

    await check("data", err.invalidAbbreviatedAmountValue).custom((input) => {
    ...
    response.data = {
        ...
        attributes: {
            ...
            "abbreviatedFormat": abbreviatedFormat != null ? abbreviatedFormat : settingsOrError.value.abbreviatedFormat,
        }
    };
