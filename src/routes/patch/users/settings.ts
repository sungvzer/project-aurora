...
export const patchUserSettings = async (
    ...
    if (settingsOrError.isError()) {
        return res
            .status(400)
            .json(response.addError(settingsOrError.error).close());
    }
    ...
    if (currency) {
        ...
        sql +=
            " UserCurrencyID = (SELECT CurrencyID FROM Currency WHERE CurrencyCode = ?) ";
    ...
    response.data = {
        ...
        attributes: {
            darkMode:
                darkMode != null ? darkMode : settingsOrError.value.darkMode,
    ...
    return res
        .status(201)
        .header("Location", `/users/${userId}/settings`)
        .json(response.close());
};
